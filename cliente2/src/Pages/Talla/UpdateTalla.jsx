...
export const UpdateTalla = () => {
    ...
    const onSubmit = data => {
        ...
        Axios.put("http://localhost:3001/updatetalla", { ...data,id:id})
        return navigate("/listastalla")
    };
    const cargarPersona = () => {
        Axios.get(`http://localhost:3001/talla/${id}`).then((response) => {
            reset(response.data);
        });
    };
    useEffect(() => {
