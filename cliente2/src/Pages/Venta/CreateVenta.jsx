...
export const CreateVenta = () => {
   ...
   const [Buscar,setBucar]=useState('');
    ...
    const onSubmit =(data) => {
        ...
        Axios.post("http://localhost:3001/createventa", data).then((response)=>{
            sendListaMateriaPrima.map((obj)=>{
                ...
                if(obj.idVenta!==undefined){
                    Axios.post("http://localhost:3001/createdetalleventa", obj)
                    Axios.post("http://localhost:3001/createpago", obj)
                }
            })
        })
